...
Created on Wed Oct  1 01:01:43 2019

@fork_vision:DUKE HOUSE
"""
...
kernel = np.ones((15, 15), np.uint8)#unused kernel
kernel2 = np.ones((1, 1), np.uint8)#unused kernel
skinkernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (1, 1))
...
def skinMask(frame, x0, y0, width, height, framecount, plot):
    ...
    cv2.rectangle(frame, (x0, y0), (x0 + width, y0 + height), (0, 255, 0), 1)# vedio src &start point & width and height & color
    ...
    if saveImg == True:
        ...
        layer = int(input("Enter which layer to visualize1 "))
        cv2.waitKey(0)
...
def bkgrndSubMask(frame, x0, y0, width, height, framecount, plot):
    ...
    if saveImg == True:
        ...
        layer = int(input("Enter which layer to visualize3 "))
        cv2.waitKey(0)
        myNN.visualizeLayers(mod, res, layer)
        visualize = False
...
def Main():
    ...
    while True:
        ...
        if ans == 2:
            ...
            layer = int(input("Enter which layer to visualize4"))
